-- Structural blocks such as Hero, FAQ and CTA are stored in dedicated tables.
-- Internal-link minimums scale safely while a new topic cluster is growing.
  ...
  select exists (
    ...
    where ts.template_id = new.template_id
      ...
      and definition.slug not in (
        'hero','quick-summary','parent-tips','faq','related-topics','cta'
      )
      and ds.id is null
