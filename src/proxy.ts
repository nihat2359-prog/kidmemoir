...
export const config = {
  matcher: [
    "/((?!api|auth/callback|auth/login|auth/recovery-prepare|auth/recovery-confirm|dev(?:/|$)|_next/static|_next/image|favicon.ico|manifest.webmanifest|robots.txt|sitemap|.*\\.(?:xml|svg|png|jpg|jpeg|gif|webp)$).*)",
  ],
};
